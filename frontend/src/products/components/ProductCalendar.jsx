...
export const ProductCalendar = ({ fechasReservadas, instrumento }) => {
  ...
  const validateRange = (range) => {
    const [start, end] = range;
    if (!start || !end) return false; // Agregar verificación para start y end

    // Convertir las fechas de range a strings para comparar con reserved
    const startStr = start.format();
    const endStr = end.format();

    const isRangeValid = !reserved.some(
      ([reservedStart, reservedEnd]) =>
        (startStr >= reservedStart && startStr <= reservedEnd) ||
        (endStr >= reservedStart && endStr <= reservedEnd) ||
        (startStr <= reservedStart && endStr >= reservedEnd)
    );
        ...
        <Grid container justifyContent={"center"} sx={{ marginTop: "1rem" }}>
          {!isSmallScreen ? (
            <Calendar
              ...
              minDate={new DateObject()}
              onChange={(range) => {
                if (range.length === 2 && validateRange(range)) {
                  setValues(range);
